...
python scripts/simulate.py     # writes worldcup_{stage}_{strength}.json (default 10000 runs; --simulations N), docs/predictions.md, and docs/evaluation-results.md
python scripts/evaluate.py     # writes match_metrics.json, simulation_metrics.json, and docs/evaluation-results.md
```
...
- `model/artifacts/evaluation/` — `match_metrics.json` (match-level WC 2022/2026 backtest) and `simulation_metrics.json` (2026 Monte Carlo scores). See [evaluation-metrics.md](evaluation-metrics.md). Regenerated by `python scripts/evaluate.py`.
- `model/artifacts/predictions/` — stage-reach and win probabilities, `worldcup_{stage}_{strength}.json`

Pre-built files under `model/artifacts/` are committed so the dashboard runs without re-simulating; regenerate with `train.py` / `simulate.py`.

### Refreshing data during the World Cup tournament

Re-fetch World Cup JSON, then ingest / train / simulate:
